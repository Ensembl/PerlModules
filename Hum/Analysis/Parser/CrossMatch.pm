...
sub next_Feature {
    ...
    while (<$fh>) {
        if ( $_ =~ /\(\d+\)/ && $_ !~ /[GATC]\(\d+\)/ ) {
            my $new_feature = $self->new_Feature_from_coordinate_line($_) or confess "No new feature returned";
            if ($feature) {
...
sub discrep_list {
    ...
    if ($line) {
        ...
        $self->{'_discrep_list'}->{ $data[3] } = $data[0];

    }
    else {
        return $self->{'_discrep_list'};
    }
...
4964  0.22 0.04 0.22  NM_002460.1       1  5065 (0)    IRF4-001       22  5077 (254)  
