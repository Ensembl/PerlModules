...
use Carp;
use YAML;
use Hum::ProjectDump::EMBL::Unfinished;
...
sub make_embl {
    ...
    $pdmp->add_FT( $embl, $seqlength, $binomial,
                   $chr, $libraryname );
    $embl->newXX;
...
sub add_Headers {
    ...
    my @comment_lines = (
        ...
        "consists of ".$pdmp->supercontig_count." supercontigs. Sequence ".
...
sub chromosome {
    ...
    unless ($pdmp->{'_chromosome'}) {
        $pdmp->{'_chromosome'} = 
            Hum::Tracking::clone_to_chromosome_from_parent_project($pdmp->project_name);
    }
...
=head1 NAME - Hum::ProjectDump::EMBL::Pooled
